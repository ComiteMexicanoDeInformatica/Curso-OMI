...
SET TARGET=libinteractive\Bucket\Bucket.exe
SET SOURCES=Bucket.cpp libinteractive\Bucket\Bucket_entry.cpp
call:recompile %TARGET% %SOURCES%
IF "%RECOMPILE%" == "1" (
<NUL SET /p elidenewline=Compiling %TARGET%... 
%GXX% -std=c++11 -o %TARGET% %SOURCES% -lm -O2 -g  -Wno-unused-result -Ilibinteractive\Bucket
IF !ERRORLEVEL! NEQ 0 EXIT /B !ERRORLEVEL!
ECHO DONE
)

SET TARGET=libinteractive\Bucket\Bucket_debug.exe
SET SOURCES=Bucket.cpp libinteractive\Bucket\Bucket_entry.cpp
call:recompile %TARGET% %SOURCES%
IF "%RECOMPILE%" == "1" (
<NUL SET /p elidenewline=Compiling %TARGET%... 
%GXX% -std=c++11 -o %TARGET% %SOURCES% -lm -g  -Wno-unused-result -Ilibinteractive\Bucket
IF !ERRORLEVEL! NEQ 0 EXIT /B !ERRORLEVEL!
ECHO DONE
)
