...
static struct __stream __pila_in, __pila_out;

void __NOINLINE __libinteractive_init() {
	openstream(&__pila_in, "\\\\.\\pipe\\libinteractive_Main_pila_in", O_WRONLY);
	openstream(&__pila_out, "\\\\.\\pipe\\libinteractive_Main_pila_out", O_RDONLY);
}
...
static void __message_loop_pila(int __current_function, int __noreturn) {
	uint32_t __msgid;
	while (readfull(&__pila_out, &__msgid, sizeof(__msgid), 0)) {
...
void agrega(int n) {
	const uint32_t __msgid = 0xc9bd7d95u;
	const uint32_t __cookie = 0x4cb8e4b0u;

	writefull(&__pila_in, &__msgid, sizeof(__msgid));
	writefull(&__pila_in, &n, sizeof(int));
	writefull(&__pila_in, &__cookie, sizeof(__cookie));
	writeflush(&__pila_in);

	__message_loop_pila(__msgid,  0 );
	uint32_t __cookie_result = 0;
	readfull(&__pila_out, &__cookie_result, sizeof(__cookie_result), 1);
...
int saca() {
	const uint32_t __msgid = 0xb495524du;
	const uint32_t __cookie = 0xcef5b3du;
	...
	__message_loop_pila(__msgid,  0 );
	int __ans = 0;
	readfull(&__pila_out, &__ans, sizeof(__ans), 1);
	uint32_t __cookie_result = 0;
	readfull(&__pila_out, &__cookie_result, sizeof(__cookie_result), 1);
