...
void move(int c) {
  if (e[camino.top()][1] != c && e[camino.top()][2] != c) {
    printf(
        "No obtuviste porcentaje del caso. Quisiste ir a un cuarto "
        "inalcanzable desde donde estaba el personaje");
    exit(0);
  }
...
void regresa() {
  if (camino.size() <= 1) {
    printf(
        "No obtuviste porcentaje del caso. Quisiste regresar cuando ya no se "
        "podia");
    exit(0);
  }
  camino.pop();
...
int main() {
  ...
  if (camino.empty() || camino.top() != fin) {
    printf(
        "No obtuviste porcentaje del caso. El personaje no termino en el "
        "cuarto adecuado");
    return 0;
  }
  ...
  if (num_moves <= num_nodos) {
    printf(
        "Felicidades, Obtuviste todo el porcentaje del caso. Hiciste %d "
        "moves()",
        num_moves);
  }
  if (num_moves > num_nodos && num_moves <= 2 * num_nodos) {
    printf("Obtuviste la mitad del porcentaje del caso. Hiciste %d moves()",
           num_moves);
  }
