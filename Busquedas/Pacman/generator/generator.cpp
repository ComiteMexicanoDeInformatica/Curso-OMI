...
int getPosFila(int pos, int columnas)
{
    return ((pos-1)/columnas);
}
int getPosColumna(int pos, int columnas)
{
    return ((pos-1)%columnas);
}
int main()
{
    ...
    for (int n=0; n<10; ++n)
    {
        ...
        pasos[getPosFila(posPacman,columnas)][getPosColumna(posPacman,columnas)] = 0;
        ...
        while(!pendientes.empty())
        {
            ...
            if(f>=1)
            {
                of =-1;
                oc = 0;
                if(mapa[(f+of)][(c+oc)]=='.' && pasos[(f+of)][(c+oc)]>(filas*columnas))
                {
        ...
        for(int i = 0 ; i< filas; i++)
        {
            for(int j = 0 ; j<columnas; j++)
            {
                ...
                else if(pasos[i][j]>distComida && mapa[i][j]=='.' && distComida!=-1)
                {
                    mapa[i][j] = 'F';
                }
            }
