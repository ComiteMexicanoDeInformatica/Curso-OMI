...
SET TARGET=libinteractive\quicksort\quicksort.exe
SET SOURCES=quicksort.cpp libinteractive\quicksort\quicksort_entry.cpp
call:recompile %TARGET% %SOURCES%
IF "%RECOMPILE%" == "1" (
<NUL SET /p elidenewline=Compiling %TARGET%... 
%GXX% -std=c++11 -o %TARGET% %SOURCES% -lm -O2 -g  -Wno-unused-result -Ilibinteractive\quicksort
IF !ERRORLEVEL! NEQ 0 EXIT /B !ERRORLEVEL!
ECHO DONE
)

SET TARGET=libinteractive\quicksort\quicksort_debug.exe
SET SOURCES=quicksort.cpp libinteractive\quicksort\quicksort_entry.cpp
call:recompile %TARGET% %SOURCES%
IF "%RECOMPILE%" == "1" (
<NUL SET /p elidenewline=Compiling %TARGET%... 
%GXX% -std=c++11 -o %TARGET% %SOURCES% -lm -g  -Wno-unused-result -Ilibinteractive\quicksort
IF !ERRORLEVEL! NEQ 0 EXIT /B !ERRORLEVEL!
ECHO DONE
)
