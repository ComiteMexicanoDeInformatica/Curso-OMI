...
**Este es un problema interactivo**, por lo que no tendrás que leer la entrada ni imprimir la salida, sino implementar en tu código la función `Player1(int x)` y podrás mandar a llamar la función del evaluador `siguienteCuarto(int c,int n)`.

# Implementación

### Tu función `Player1(int x)`

C/C++ `void Player1(int x);`

### Descripción

El evaluador buscará en tu código esta función y la llamará con un entero $x$ donde $1$ $\leq$ N $\leq$ $10^{5}$ .
