...
static void __message_loop_ordenador(int __current_function, int __noreturn) {
	...
	while (readfull(&__ordenador_out, &__msgid, sizeof(__msgid), 0)) {
		...
		switch (__msgid) {
			...
			case 0xcc484du: {
				// ordenador -> Main.obten
				uint32_t __cookie;
				int obten_indice;
				readfull(&__ordenador_out, &obten_indice, sizeof(int), 1);
				readfull(&__ordenador_out, &__cookie, sizeof(__cookie), 1);
			...
			case 0xf750b6fu: {
				// ordenador -> Main.desplaza
				uint32_t __cookie;
				int desplaza_indice;
				readfull(&__ordenador_out, &desplaza_indice, sizeof(int), 1);
				readfull(&__ordenador_out, &__cookie, sizeof(__cookie), 1);

				bool result =
				desplaza(desplaza_indice);
			...
			case 0x88dba7d3u: {
				// ordenador -> Main.inserta
				uint32_t __cookie;
				int inserta_indice;
				readfull(&__ordenador_out, &inserta_indice, sizeof(int), 1);
				readfull(&__ordenador_out, &__cookie, sizeof(__cookie), 1);

				inserta(inserta_indice);
...
void ordena(int n) {
	const uint32_t __msgid = 0x27faead1u;
	const uint32_t __cookie = 0xf0a1b4d1u;
