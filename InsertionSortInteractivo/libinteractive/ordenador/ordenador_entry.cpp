...
static void __message_loop_Main(int __current_function, int __noreturn) {
	...
	while (readfull(&__in, &__msgid, sizeof(__msgid), 0)) {
		...
		switch (__msgid) {
			case 0x27faead1u: {
				// Main -> ordenador.ordena
				uint32_t __cookie;
				int ordena_n;
				readfull(&__in, &ordena_n, sizeof(int), 1);
...
int siguiente() {
	const uint32_t __msgid = 0x7edc6c5u;
...
int obten(int indice) {
	const uint32_t __msgid = 0xcc484du;
	const uint32_t __cookie = 0x18be4689u;
...
bool desplaza(int indice) {
	const uint32_t __msgid = 0xf750b6fu;
	const uint32_t __cookie = 0x223dcef9u;
...
void inserta(int indice) {
	const uint32_t __msgid = 0x88dba7d3u;
	const uint32_t __cookie = 0x531f7044u;
