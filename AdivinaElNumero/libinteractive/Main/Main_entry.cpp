...
void __NOINLINE __libinteractive_init() {
	openstream(&__AdivinaElNumero_in, "\\\\.\\pipe\\libinteractive_Main_AdivinaElNumero_in", O_WRONLY);
	openstream(&__AdivinaElNumero_out, "\\\\.\\pipe\\libinteractive_Main_AdivinaElNumero_out", O_RDONLY);
}
	...
	while (readfull(&__AdivinaElNumero_out, &__msgid, sizeof(__msgid), 0)) {
		...
		switch (__msgid) {
			case 0x2aaddccbu: {
				...
				uint32_t __cookie;
				long long pista_n;
				readfull(&__AdivinaElNumero_out, &pista_n, sizeof(long long), 1);
...
void adivina(long long a, long long b) {
	const uint32_t __msgid = 0x1a36180bu;
	const uint32_t __cookie = 0x70046981u;
