...
long long pista(long long n) {
	const uint32_t __msgid = 0x2aaddccbu;
	const uint32_t __cookie = 0x70046981u;

	writefull(&__out, &__msgid, sizeof(__msgid));
	writefull(&__out, &n, sizeof(long long));
	writefull(&__out, &__cookie, sizeof(__cookie));
	writeflush(&__out);
...
int main(int argc, char* argv[]) {
	...
	openstream(&__in, "\\\\.\\pipe\\libinteractive_AdivinaElNumero_AdivinaElNumero_in", O_RDONLY);
	openstream(&__out, "\\\\.\\pipe\\libinteractive_AdivinaElNumero_AdivinaElNumero_out", O_WRONLY);
