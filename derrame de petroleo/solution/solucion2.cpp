...
bool estadoValido(estado edo)
{
    ...
    if(edo.fila<0 || edo.fila>=R || edo.columna<0 || edo.columna>=C)
        return false;
    /**Si es una pared '#', el inicio '$' o una casilla que ya fue explorada '*', no es válido**/
 ...
 **/
 ...
 while(!nodos.vacio())
 {
     /**extraer el siguiente nodo de la cola, generar sus hijos e insertarlos, solo si son estados válidos**/
